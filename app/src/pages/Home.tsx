...
export default function Home() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      await deriveKeyPBKDF2(trimmedPassphrase, room.saltB64, room.kdfIters);
      sessionStorage.setItem(
        `recipe:${room.roomId}`,
        JSON.stringify({ passphrase: trimmedPassphrase, version: room.version })
      );
      ...
      <div className="home-content">
        ...
        <form className="home-form" onSubmit={handleSubmit}>
          <div>
            ...
            <input id="recipe-name" type="text" value={recipeName} onChange={(e) => setRecipeName(e.target.value)} placeholder="Recipe name" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" />
          </div>
          <div>
            <label htmlFor="passphrase">Passphrase</label>
            <input id="passphrase" type="password" value={passphrase} onChange={(e) => setPassphrase(e.target.value)} placeholder="Passphrase" autoComplete="off" />
          </div>
            ...
            <ul className="recent-rooms-list">
              {recentRooms.map((room) => (
                <li key={room.roomId} className="recent-room-item">
                  <button className="recent-room-link" onClick={() => handleRecentClick(room)}>
                    <span className="recent-room-name">{room.title}</span>
                    <span className="recent-room-id">{room.roomId}</span>
                  </button>
                  <button className="recent-room-remove" onClick={() => handleRemoveRecent(room.roomId)} title="Remove">&times;</button>
                </li>
              ))}
            </ul>
          </div>
